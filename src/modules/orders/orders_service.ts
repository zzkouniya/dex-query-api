...
export default class OrdersService {
  ...
  async getOrders(
    ...
    dexOrdersBid.forEach(x => {    
      if(!groupbyPriceBid.has(x.price.toString())) {
        return;
      }
      const key = CkbUtils.roundHalfUp(x.price.toString()).toString();
      
      if(!bidOrderPriceMergeKeys.has(key))  {
        bidOrderPriceKeys.push(x.price.toString());
    ...
    dexOrdersAsk.forEach(x => {
      if(!groupbyPriceAsk.has(x.price.toString())) {
        return;
      }

      const key = CkbUtils.roundHalfUp(x.price.toString()).toString();
      if(!askOrderPriceMergeKeys.has(key))  {
        askOrderPriceKeys.push(x.price.toString());
  ...
  groupbyPrice(dexOrders: DexOrderCellFormat[]): Map<string, DexOrderCellFormat[]> {
    ...
    for(let i = 0; i < dexOrders.length; i++) {
      ...
      let priceArr = groupbyPrice.get(dexOrder.price.toString());
      if(!priceArr) {
        priceArr = []
        groupbyPrice.set(dexOrder.price.toString(), priceArr);
      }
